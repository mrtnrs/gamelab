...
import { FiPlus, FiGrid, FiUsers, FiTrendingUp, FiActivity, FiAlertCircle, FiSend } from 'react-icons/fi'
import { GameSubmission } from '@/services/submission-service'
...
export default function DashboardClient() {
  ...
  // Load game submissions
  useEffect(() => {
    const fetchSubmissions = async () => {
      setLoadingSubmissions(true)
      try {
        const response = await fetch('/api/admin/submissions?limit=5');
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        <div className="bg-card rounded-lg shadow-sm border border-border">
          <div className="px-6 py-4 border-b border-border">
            <h2 className="text-xl font-semibold">Game Submissions</h2>
          </div>
          <div className="divide-y divide-border">
            {loadingSubmissions ? (
                ...
                <div key={submission.id} className="px-6 py-4">
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {submission.email} • <a href={submission.link_to_socials} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Social Link</a>
                      </p>
                    </div>
                    <div className="text-sm text-muted-foreground">
